...
import vis from 'vis'
const MainTopology = React.createClass({
  render () {
    return (
      <div id='mynetwork' style={{width: this.state.width+'px', height: this.state.height+'px'}}></div>
    )
  },
  ...
  componentDidMount() {
    ...
    this.setState({width:document.getElementById('J_Topology').offsetWidth,height:document.getElementById('J_Topology').offsetHeight})
    var color = 'gray';
    // var len = undefined;
    ...
    // let network = new vis.Network(container, data, options);
     var container = document.getElementById('mynetwork');
    var DOTstring = 'dinetwork {1 -> 2 -> 3; 2 -> 4 -> 5; 4 -> 6;7->8->9; 10->11->12}'
    var parsedData = vis.network.convertDot(DOTstring)
    var data = {
      nodes:parsedData.nodes,
      edges:parsedData.edges
    }
    let network = new vis.Network(container,data,parsedData.options)
